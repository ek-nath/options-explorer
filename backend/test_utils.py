...
class TestUtils(unittest.TestCase):
    def test_black_scholes_robustness(self):
        ...
        # Test sigma = 0
        sigma = 0
        price, delta, gamma, theta, vega, vanna, charm = black_scholes(S, K, T, r, sigma)
        ...
        T_zero = 0
        sigma_val = 0.2
        price, delta, gamma, theta, vega, vanna, charm = black_scholes(S, K, T_zero, r, sigma_val)
        self.assertEqual(price, 0) # At the money, T=0, call price should be 0 (intrinsic)
        self.assertEqual(gamma, 0)
        
        # Test very small T
        T_small = 1e-9
        price, delta, gamma, theta, vega, vanna, charm = black_scholes(S, K, T_small, r, sigma_val)
    ...
    def test_black_scholes_values(self):
        ...
        price, delta, gamma, theta, vega, vanna, charm = black_scholes(S, K, T, r, sigma, option_type='call')
        ...
        # Check second order greeks are calculated
        self.assertIsNotNone(vanna)
